...
export default function DashboardPage() {
  const tickers = useQuery({
    ...
    queryFn: async () => {
      ...
      const results = await Promise.all(symbols.map((s) =>
        api.get<Ticker>(`/market/ticker/${encodeURIComponent(s)}`).then((r) => r.data),
      ));
      return results;
    },
  ...
  const candles = useQuery({
    ...
    queryFn: async () => {
      ...
      return r.data.candles.map((c) => ({ ts: c.ts.slice(5, 16), close: parseFloat(c.close) }));
    },
  ...
  const movers = useQuery({
    ...
    queryFn: async () => {
      const r = await api.get<{ direction: string; movers: Mover[] }>(
        "/market/movers",
        { params: { direction: "up", n: 8 } },
      );
  ...
  const audits = useQuery({
    ...
    queryFn: async () => {
      const r = await api.get<{ items: { id: string; chain: string; address: string;
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          {tickers.data?.map((t) => (
            <div key={t.symbol} className="bg-slate-900/40 rounded-lg p-3 border border-white/5">
              ...
              <div className="text-lg font-mono">{formatNumber(t.price, 2)}</div>
              <div className="text-[10px] text-slate-500">{formatRelative(t.ts)}</div>
            </div>
          ))}
        </div>
      </Card>

      <Card title="Dominance" subtitle="Snapshot quotes / quote currency">
            ...
            Object.entries(dominance.data.values)
              ...
              .map(([k, v]) => (
                <li key={k} className="flex items-center justify-between">
                  <span>{k}</span>
                  <Badge tone="primary">{(v * 100).toFixed(1)}%</Badge>
                </li>
              ))}
        </ul>
      </Card>
        ...
        <ul className="space-y-2">
          {movers.data?.map((m) => (
            <li key={m.symbol} className="flex items-center justify-between text-sm">
              <span>{m.symbol}</span>
              <Badge tone={m.change_pct > 0 ? "bullish" : "bearish"}>
            ...
            <tbody>
              {audits.data?.map((a) => (
                <tr key={a.id} className="border-t border-white/5">
                  ...
                  <td className="py-2 text-slate-400 text-xs">{formatRelative(a.created_at)}</td>
                </tr>
              ))}
            </tbody>
          </table>
