...
PATTERNS: list[tuple[str, str, str, str]] = [
    ...
    (
        ...
        "SOL_SELFDESTRUCT",
        "critical",
        "selfdestruct() found in source",
    ),
    (
        r"\.delegatecall\s*\(",
        "SOL_DELEGATECALL",
        "high",
        "delegatecall() with potential attacker-controlled target",
    ),
    (
        r"tx\.origin",
        "SOL_TX_ORIGIN",
        "high",
        "tx.origin used for authorisation — phishing vector",
    ),
    ...
    (
        ...
        "SOL_PUBLIC_MINT",
        "medium",
        "Public mint() function — potential supply inflation",
    ),
    (
    ...
    (
        ...
        "SOL_BLACKLIST",
        "high",
        "Blacklist add function present",
    ),
    (
        r"function\s+setFee\s*\(",
        "SOL_FEE_FUNCTION",
        "medium",
        "Mutable fee setter present",
    ),
    ...
    (
        ...
        "SOL_FORCE_BALANCE",
        "critical",
        "Direct balance manipulation in source",
    ),
    (
        r"require\s*\(\s*sender\s*==\s*owner",
        "SOL_OWNER_GUARD",
        "info",
        "Owner-only sender guard",
    ),
    (
        r"\.transfer\s*\(\s*\d+\s*\)",
        "SOL_HARDCODED_TRANSFER",
        "low",
        "Hard-coded transfer amount",
    ),
    (
        r"abi\.encodePacked\s*\(",
        "SOL_ENCODE_PACKED",
        "info",
        "abi.encodePacked() used (collision risk)",
    ),
    (
        r"unchecked\s*\{",
        "SOL_UNCHECKED_BLOCK",
        "info",
        "unchecked block — overflow review",
    ),
    (
    ...
    (
        ...
        "SOL_NEW_DEPLOY",
        "info",
        "Deploys child contracts via 'new'",
    ),
    (
        r"keccak256\s*\(\s*abi\.encodePacked\s*\(\s*[^,]*,[^,]*,[^,]*\)\)",
        "SOL_KECCAK_PACKED_MULTI",
        "medium",
        "keccak256(encodePacked(multiple args)) — collision risk",
    ),
]
